...
func appErrorUnauthorized(err error, msg string) *appError {
	return &appError{err, msg, http.StatusUnauthorized}
}

func appErrorForbidden(err error, msg string) *appError {
	return &appError{err, msg, http.StatusForbidden}
}
...
func writeJSON(w http.ResponseWriter, contype string, content interface{}) *appError {
	...
	if err != nil {
		...
		return appErrorInternal(err, "Error encoding JSON")
	}
...
func writeHTML(w http.ResponseWriter, content interface{}, context interface{}, templ *template.Template) *appError {
	...
	if err != nil {
		...
		return appErrorInternal(err, "Error encoding HTML")
	}
	return writeResponse(w, "text/html; charset=utf-8", encodedContent)
}

// checkUIDisabled returns an error if HTML UI is disabled
func checkUIDisabled(format string) *appError {
	if format == "html" && conf.Configuration.Server.DisableUi {
