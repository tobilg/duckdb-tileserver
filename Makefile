...
##      PROGRAM - Name of binary, duckdb-tileserver
...
APPVERSION ?= latest
GOVERSION ?= 1.24.6
PROGRAM ?= duckdb-tileserver
CONTAINER ?= tobilg/$(PROGRAM)
...
.PHONY: build bin-for-docker build-common build-in-docker check clean docker install multi-stage-docker release set-local set-multi-stage test uninstall
...
$(PROGRAM): $(GOFILES) 
	go build -v -ldflags "-s -w -X github.com/tobilg/duckdb-tileserver/conf.setVersion=$(APPVERSION)"
...
# to be used in docker the built binary needs the CGO_ENABLED=0 option
	CGO_ENABLED=0 go build -v -ldflags "-s -w -X github.com/tobilg/duckdb-tileserver/conf.setVersion=$(APPVERSION)"
...
release: clean docker  ##            Generate a local build, and then uses the local build to generate BASE_IMAGE container
...
install: $(PROGRAM)  ##            This will install the program locally
	...
	$(SED) 's,./assets,/usr/share/$(PROGRAM)/assets,' config/$(PROGRAM).toml.example > $(DESTDIR)/etc/$(PROGRAM).toml
